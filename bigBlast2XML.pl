...
#This script takes input blast file in usual ncbi output and change it into xml format. 
#This concrete version of the script is able to handle big files, from more then 10.000 queries to more then 20 subjects. Thats the reason not to use BioPerl, since operation in memory could dramatically affect speed of the process.
#Another thing - if xml files will be afterwards uploaded into another software, such as blast2go, then its better to split file to chunks.

#
#    bigBlast2MXL.pl is free software, under the terms of the GNU General Public License 
#
#    bigBlast2MXL.pl is distributed WITHOUT ANY WARRANTY
#
# Author : Anna Vlasova, vlasova.av .at. gmail.com
...
#start to process queries and hits, and print converted results into files.
open(INFILE, $blastFileName)||die "Can't open BLAST file for reading!\n";
...
#old line:
    ...
    print $OUTFILE "<Hit_id>$hitId</Hit_id>\n";
    $hitDef=~s/\&//;
  ...
  #new hsp
  elsif($line=~/Score\s+\=.+Expect\s+\=/)
   {
    ...
    $hspCount++;
    ($hspData{'bitScore'}, $hspData{'score'}, $hspData{'eValue'}) = $line=~/Score\s+\=\s+([0-9.]+)\s+bits\s+\((\d+)\).+Expect\s\=\s(\S+)[, ]*/;
    #identity line
    $line=<INFILE>;
...
{
 my $fileName = shift;
 my $header = `head -20 $fileName`;
 my $tail =`tail -20 $fileName`;
 ...
 foreach my $line(@dataArray)
  ...
  elsif($line=~/Number\sof\sSequences\:\s+(\S+)\s*/)
   {
    my $dbSeqs = $1;
    $dbSeqs =~s/\,//g;
